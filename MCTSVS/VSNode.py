...
class VSNode:
    ...
    def __init__(self, parent=None, dims=0, active_dims_idx=None, min_num_variables=3, reset_id=False):
        ...
        if reset_id:
            VSNode.obj_counter = 0

        self.id = VSNode.obj_counter
        VSNode.obj_counter += 1
    ...
    def split(self, split_type='median'):
        ...
        left = VSNode(parent=self, dims=self.dims, active_dims_idx=good_idx, min_num_variables=self.min_num_variables,
                    reset_id=False)
        left.init_bag(self.features, self.samples, self.feature2sample_map)
        right = VSNode(parent=self, dims=self.dims, active_dims_idx=bad_idx, min_num_variables=self.min_num_variables,
                     reset_id=False)
